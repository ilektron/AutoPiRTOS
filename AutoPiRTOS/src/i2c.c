...
uint32_t I2CStart( void )
{
	...
	// TODO: Change this to vTaskDelay
	...
	while( 1 )
	{
		...
		// Wait 1 tick so we aren't just sitting on our thumbs
//		vTaskDelay(1);
		timeout++;
	}
	return( retVal );
...
int i2c_write_byte(uint8_t address, uint8_t sub_address, uint8_t byte)
{
	I2CWriteLength = 3;
	I2CReadLength = 0;
	I2CMasterBuffer[0] = address;		/* address */
	I2CMasterBuffer[1] = sub_address;	/* register/sub address */
	I2CMasterBuffer[2] = byte;	/* register/sub address */
	I2CEngine();
...
uint8_t i2c_read_byte(uint8_t address, uint8_t sub_addr)
{
	...
	I2CMasterBuffer[1] = sub_addr;		/* address */
